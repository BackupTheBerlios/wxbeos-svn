...
class WXDLLIMPEXP_CORE wxWindowBeOS : public wxWindowBase
{
...
public:
    ...
    // Get main BeOS window
    virtual WXWindow GetMainWindow() const;
    
    // Get BeOS window representing the client area
    ...
    // has to be done in idle time in order for wxBeOS to work is done in
    // OnInternalIdle
...
//knorr
	...
	virtual void Draw();
	virtual BView * GetBView() { return (BView*)m_view; };
//knorr    
protected:
...
private:
	...
	void Init();
	DECLARE_DYNAMIC_CLASS(wxWindowBeOS)
	DECLARE_NO_COPY_CLASS(wxWindowBeOS)
	DECLARE_EVENT_TABLE()
};
