...
bool wxTextCtrl::Create(wxWindow *parent, wxWindowID id,
                        ...
                        const wxString& name)
{
	BRect frame(pos.x, pos.y, pos.x+size.x, pos.y+ size.y);
	wxPoint aPos = pos;
	if(pos==wxDefaultPosition)
		aPos = parent->GetClientAreaOrigin();
	frame.left = aPos.x;
	frame.top = aPos.y;
	...
	if(size==wxDefaultSize)
		aSize = GetSize();
	frame.right = aPos.x + aSize.x;
	frame.bottom = aPos.y + aSize.y;

	m_textview = new BTextView(frame, name.c_str(), frame, B_FOLLOW_ALL, 0);
	m_view = (BBackView *)m_textview;
	if(parent)
