...
{
//knorr meybe use find_diectory() instead??
	*home = "/boot/home/config/settings";
	return home->c_str();
//knorr
}
...
wxToolkitInfo& wxAppTraitsBase::GetToolkitInfo()
{
    static wxToolkitInfo info;
    info.os = wxBEOS;
    info.name = _T("wxBase");
...
{
	BScreen *dpy = (BScreen*) wxGetDisplay();
	if (width)
		*width = dpy->Frame().Width()+1;
	if (height)
	        *height = dpy->Frame().Height()+1;
}
...
{
	return wxColour(clr.red, clr.green, clr.blue);
}

wxRect be2wx(BRect BR)
{
	return wxRect(BR.left, BR.top, BR.IntegerWidth(), BR.IntegerHeight());
}
