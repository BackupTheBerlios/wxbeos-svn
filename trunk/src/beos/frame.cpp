/////////////////////////////////////////////////////////////////////////////
// Name:        src/beos/frame.cpp
// Purpose:     wxFrame
// Author:      William Osborne - minimal working wxPalmOS port
// Modified by: Anton Sokolov - BeOS Port
// Created:     27.12.06
// RCS-ID:      $Id: frame.cpp,v 1.10 2005/07/01 19:36:56 ABX Exp $
// Copyright:   (c) William Osborne, Wlodzimierz Skiba, Anton Sokolov
...
{
	wxMenuItem * item;
	if(!(item=m_frameMenuBar->FindItem(event->what)))
		return false;

	if(item->IsCheckable())
		item->Check(!item->IsChecked());

	wxCommandEvent commandEvent(wxEVT_COMMAND_MENU_SELECTED, event->what);
...
bool wxFrameBase::DoPopupMenu(wxMenu *menu, int x, int y)
{
//knorr Just Do Popup Menu :))
	return false;
}

void wxFrame::SetMenuBar(wxMenuBar *menubar)
{
	m_view->AddChild(menubar->GetBMenuBar());
...
wxPoint wxFrame::GetClientAreaOrigin() const
{
	...
	wxPoint origin(0, 0);
	wxMenuBar * menu = wxFrameBase::GetMenuBar();
	if(menu)
	{
		origin.x = menu->GetSize().x;
		origin.y = menu->GetSize().y;
	}
	return origin;
}
